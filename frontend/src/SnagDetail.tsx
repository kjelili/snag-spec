...
import { ArrowLeft, FileText, AlertTriangle, Clock, CheckCircle2, Sparkles, Trash2, ShieldCheck } from 'lucide-react'
...
import { canGenerateInstruction, canCorrectSnag } from '../lib/auth'
...
export default function SnagDetail() {
  ...
  const isOwner = snag && user ? snag.created_by === user.id : false
  const canGenerate = user ? canGenerateInstruction(user) : false
  const canClose = user ? canCorrectSnag(user) || isOwner : false
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex-1">
          ...
          <p className="mt-1 text-sm text-gray-500">
            ...
            {!isOwner && (
              <span className="ml-2 text-indigo-500 font-medium">(created by another user)</span>
            )}
          </p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {!isOwner && (
            <div className="flex items-start gap-3 rounded-lg border border-indigo-200 bg-indigo-50 p-4">
              ...
              <div>
                <p className="text-sm font-medium text-indigo-800">Read-only view</p>
                <p className="text-xs text-indigo-600">
                  This snag was created by another user. You can view it but some actions may be restricted.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="app-card p-6">
            ...
            {!canGenerate && (
              <p className="text-sm text-slate-500 text-center py-2">
                Only Architects and Project Managers can generate instructions.
              </p>
            )}
